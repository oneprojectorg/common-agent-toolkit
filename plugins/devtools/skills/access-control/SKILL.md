...
## Keep the access primitives inside the access package

`resolveAccessUserIds`, the `GLOBAL_USER_PUBLIC` fold, and the role-normalization walk are implementation details of `services/access`. A feature service that reaches for them directly to hand-build a membership check spreads the authorization model across the codebase, and every copy is a place the model can drift. PR #1873 review on `proposalVisibility.ts`: *"This might be happening already elsewhere but I'd rather not leak the `resolveAccessUserIds` / public-union logic further outside the access package."*

The same thread carries the check to run before you write one: *"aren't we in proposal land here? The public grant lives at the decision instance level, so the union buys nothing in this membership check."* Fold the public sentinel at the level the grant is actually made; folding it a level down adds a branch that can never be true, and it reads as a rule.

**Reach for the existing domain assert before writing a bespoke check.** PR #1851 on `getExportStatus`: *"We can use `assertInstanceProfileAccess` instead although we can skip the organization fallback. Just to keep it consistent with patterns and to singularly locate that logic. Alternatively, we could also just gate this on the owner since there is no current way to view another person's export."* Two acceptable answers — the shared assert, or an explicit owner check — and one unacceptable one, which is a third hand-rolled variant of the same lookup. The same review shape landed on `reviewHelpers.ts` in #1815: *"Couldn't we just keep to the pattern of asserting on read access here?"*
