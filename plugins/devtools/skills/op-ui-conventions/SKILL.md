...
description: Use the @op/ui component library, design tokens (primary-teal, neutral-gray), and the type scale (text-title-lg, text-sm) instead of native HTML, hex colors, or raw Tailwind sizes/arbitrary values (stroke-[1.5]). Prefer semantic dark-mode-aware tokens (bg-muted, text-muted-foreground) over fixed palette values (bg-gray-100). Tailwind v4 traps — a bare number in a typed utility (duration-450) silently emits no CSS, use duration-[450ms]; conflicting utilities in one class string are discarded by tailwind-merge; a child's opacity-100 cannot undo an ancestor's opacity-50. Sentence-case labels matched to siblings. Accessibility and semantics — aria-label replaces (never supplements) visible text, pointer-events-none is not disabled, inert items must not be keyboard-reachable, don't render an affordance the component can't honor, a stateful control's label must track its state, aria-sort on sortable headers, render an anchor only with a real href else a div, default dir="auto", role="tabpanel" only when a paired tab exists. next/image in a CSS-sized container uses fill + object-cover, and an absent URL is `?? undefined` not `?? ''`. Keep @op/ui framework-agnostic (no next/image inside packages/ui — inject it via a render prop; package components take copy via a single i18n-agnostic prop) and use semantic heading hierarchy (the hero title is the h1). Use when writing JSX/TSX, picking a color, font size, duration, or stroke width, casing a label, setting an aria-label or dir, wiring a loading/disabled state, importing from @op/ui, choosing a variant vs creating a one-off, adding an image to a component, setting heading levels, or editing packages/ui or packages/sense.
---
...
- **`aria-label` REPLACES the visible text — never use it to bolt on extra information.** A screen-reader user gets only the label, so `aria-label={reviewerNames}` on a trigger whose visible text reads "3 Reviewed" announces "Alice, Bob" with no hint of what the control is. When you want to add detail rather than rename, let the tooltip / popover content be the accessible description, or wire `aria-describedby` — both the label and the detail get through. PR #1711.
- **`pointer-events-none` is not `disabled`.** It blocks mouse and touch but not Tab-then-Space/Enter, and `aria-busy` is purely informational — so a "loading" button styled that way is still keyboard-activatable and will double-fire for any caller that doesn't separately guard on `isPending`. Set the native `disabled` on the underlying element (`disabled={loading || undefined}`) so the browser's own not-actionable semantics apply. Keep `pointer-events-none` only for the link-rendered case (`render={<a>}`), since an anchor ignores `disabled`, and pair it with a readable disabled opacity so the spinner doesn't grey out. PR #1670.
- **An inert item must not be keyboard-reachable.** Interactive-list primitives (`cmdk`'s `CommandItem`, menu items, rows) are focusable and Enter-activatable by default; if the handler early-returns for some subset (a profile the viewer can't open), the user lands on it, presses Enter, and gets silence — no navigation, no feedback, the dropdown just sits there. Pass `disabled` for that subset so keyboard navigation skips it entirely. PR #1669: "In the old code, non-member profiles were wrapped in a plain `<div>` and weren't reachable via keyboard at all."
- **Don't render an affordance the component can't honor.** A `showDragHandle` that defaults to `true` renders a real `<button>` with `cursor-grab` and `aria-label="Drag to reorder"` even when no `dragHandleProps` were passed — AT users tab into it and activate it to no effect, and every caller has to remember to opt out. Default the affordance **off**, or derive it from the props that make it work (`Boolean(dragHandleProps)`), so the safe path is the default path. PR #1659.
- **A stateful control's visible label must track its state.** When a toggle's label is the only signal of what it's doing, it has to change with `isSelected` — `{isSelected ? t('Advancing') : t('Advance')}`. PR #1711 caught the label frozen at "Advance" after a Toggle migration, "making it impossible for sighted users to tell whether a proposal is advancing" (the invisible width-reservation placeholder was still sized for the string the button no longer rendered).
- **Sortable column headers need `aria-sort`.** Rendering the direction as an arrow icon leaves screen-reader users with no way to know which column is sorted or how. Set `aria-sort` to `"ascending"` / `"descending"` / `"none"` on the `<th>` — the `active` flag and `sortDescriptor.direction` you already use for the arrow give you the value. PR #1702.
...
- **The rule isn't just about sizing — it's every utility.** `stroke-[1.5]` got flagged twice (PRs #1673, #1726) because Tailwind ships only `stroke-0` / `stroke-1` / `stroke-2`: "the arbitrary `stroke-[1.5]` utility instead of a design-system utility, bypassing the repository's Tailwind token convention and making icon styling harder to maintain consistently." Snap to an existing tier, or — if the off-scale value is genuinely load-bearing, as it is for an app-wide icon stroke — add a CSS token (`--stroke-icon`) and reference it (`stroke-[var(--stroke-icon)]`) so there's one place to change it.

## Tailwind v4 — a bare number in a typed utility silently produces nothing

Tailwind v4 resolves `duration-*` through `--value(--duration-*, [time])`: it looks for `--duration-<n>` in the `@theme`, and failing that tries to parse the bare value as a CSS `[time]`. A unitless integer is not a valid time, so **`duration-450` matches nothing, generates no CSS, and is dropped at build with no warning** — the transition runs instantly. Use the bracket arbitrary-value form with an explicit unit: `duration-[450ms]`.

PR #1688 shipped this on both the drawer overlay and the popup: "`duration-450` will silently produce no CSS in this project … The class is dropped at build time, so the overlay fade will be instant rather than the intended 450 ms." The failure is invisible in code review and in a typecheck — the only tell is that the animation doesn't animate. The same trap applies to any theme-keyed utility given an off-theme bare value; when a visual change is the whole point of the PR, open the page and confirm the CSS actually landed.

**Delete conflicting utilities from a base class string.** `tailwind-merge` silently discards the loser when two utilities in the same group collide (`cursor-default` alongside `cursor-pointer`), so the dead class has no runtime effect but reads as intentional to the next contributor. PR #1671: "Remove the dead `cursor-default` — it conflicts with `cursor-pointer` in the same class string and `tailwind-merge` always discards it."

**A child cannot un-dim an opacity-dimmed ancestor.** CSS `opacity` composites the entire parent render-buffer before display, so `opacity-100` on a child of an `opacity-50` wrapper is a no-op — the child still renders at 50%. To keep one element bright inside a muted block, dim the siblings with color tokens (`text-muted-foreground`) instead of putting an opacity layer on the shared wrapper. PR #1675.
...
## `next/image` in a CSS-sized container — `fill` + `object-cover`

When the container's size comes from CSS (a responsive `size-8 md:size-12` avatar), pass `fill` with `object-cover` rather than fixed `width` / `height`. Fixed intrinsic dimensions pin the requested asset to one size, so a `width={48}` avatar in a 32 px mobile container over-fetches, and without `object-cover` a non-square source crops badly. PR #1670: every other `ProfileAvatar` in the diff used `fill` + `object-cover`; the one holdout with `width={48} height={48}` sat in a `size-8` container — "Next.js will still request/serve a 48 px image and the avatar crops."

Related, from the same review: **use `?? undefined`, not `?? ''`, for an absent image URL.** An empty string is a non-null `string`, so it satisfies the type while meaning "no URL" — it only works because the consumer happens to falsy-check. `undefined` says what's true and matches the sibling avatar helpers.
