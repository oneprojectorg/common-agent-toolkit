...
description: Channel naming and design — the Channels.X builders in packages/common/src/realtime/channels/channels.ts (scope[:id] convention), JSDoc subscriber/broadcaster pairing, exporting a per-builder type plus the ChannelName union, when to add a channel vs reuse one, registering every input that changes a query's output (not just its main table — a gate on another table, published by someone else's mutation, still moves the answer), when NOT to over-channel a mutation (including the component-side version — a manual utils.x.y.invalidate in onSuccess on top of a channel the query already registers just makes the actor refetch twice), and the channelScope.ts helper pattern for fanning out invalidations to many subscribers. Supabase Realtime does not replay, so a broadcast published before the client's subscription lands — or during a reconnect — is lost forever; a query waiting on a background job that can finish before the join needs a re-read on subscribe, not a longer timeout, and a workflow must broadcast intermediate states or a client silence-bound degrades into a total-duration cap. Use when adding a new channel, naming a channel, deciding whether a mutation should register an existing channel or a new one, converting a poll loop to realtime, wiring a client that waits on a background job, or wiring per-profile / per-collection invalidation fan-out for a multi-tenant feature.
---
...
## A query registers every input that changes its output, not just its main table

The channels a query declares have to cover each thing that can change the answer — including the ones that live on another table and are published by a different mutation. PR #1815: `listProposalFeedback` subscribed only to `reviewAssignments`, but a phase transition changes what feedback is visible and publishes on `decisionInstance`, so advancing the phase left the panel showing the previous phase's answer until something else forced a refetch.

The check runs the other way round from the over-channelling one below: list everything that can change this query's result — the rows it reads, the gates it depends on (phase, visibility, membership), the config it resolves — then confirm a registered channel covers each. Under-registering fails silently and only in production, where someone else's mutation is the one that moves.
...
**The other over-invalidation is a component that doesn't trust the wiring.** When the query registers a channel and the mutation publishes to it, the actor's cache is already invalidated — a `utils.x.y.invalidate(...)` in `onSuccess` on top of that just makes the person who clicked refetch twice. `AssignProposalsDialog` does exactly this against `listPhaseReviewAssignments` (#1848, still on `dev`). Before adding a manual invalidate, open the query's and the mutation's procedures and compare their channel lists; if they match, delete the call. If they don't, the fix is the channel. See `component-file-structure` on cache invalidation.

**Reconsider a channel's cost when a query moves surfaces.** A channel that fires on a common mutation is cheap while it feeds an admin-only page nobody keeps open, and expensive the moment it feeds a tab a user leaves open all day over an unpaginated read. Registering the channel is correct in both cases; the read behind it is what has to change (see `service-layer-structure` on re-evaluating an unpaginated read when its trigger changes).
...
## A broadcast published before the client joins is lost forever

Supabase Realtime does not replay. A client only subscribes *after* the response that carries the channel, so anything broadcast in that window is gone — and the same hole opens on **reconnect**: if the socket drops and returns, every invalidation published in the interval is lost permanently, for every feature, with nothing to recover it.

For most queries this doesn't matter: the data was fetched at about the moment the channel was registered, so the gap holds nothing worth re-reading. It matters for **a query waiting on a background job that can settle before the join lands**. A proposal export finishing in under a second — plausible locally, where `inngest-cli dev` picks the event up immediately — broadcasts `completed` to nobody, and the client waits out its timeout and reports failure on a successful export. Under the polling this replaced, a lost broadcast self-healed on the next tick; going realtime-only removed that safety net (PR #1750).

So when you convert a poll loop to realtime, or add a channel a background job broadcasts to, ask explicitly: *can this job finish before the subscription is live?* If yes, the client needs a re-read on subscribe — a "channel subscribed" signal it can opt into — not a longer timeout. Promoting that to fire app-wide is a real design decision (a refetch per channel subscription across every realtime query, and a load increase on features the PR doesn't touch), so it belongs in its own change rather than a feature branch.

**Broadcast intermediate states, not just terminal ones.** A workflow that writes `processing` to its record and tells nobody leaves the client hearing nothing between the first read and the end of the run. Two things then break at once: the "Generating…" label never appears, and a client-side bound on *silence* degrades into a bound on *total duration* — so a long-but-healthy run gets reported as timed out. If a client has to wait on a job, give it either a broadcast per state change or a heartbeat; a fixed timeout is a guess about how long the work takes, and the guess expires the moment the work can grow.

One consequence worth designing around: a client timeout ends only the client's wait. The job keeps running and may write its artifact a second later — unreachable if the status record is cache-only with no history, so the user re-runs the whole thing.
